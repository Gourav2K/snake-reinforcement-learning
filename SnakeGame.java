        ...
        do {
            ...
            for (Tile snakePart : snakeBody) {
                if (snakePart.x == x && snakePart.y == y) {
                    isUnderSnake = true;
                    break; // Exit the loop as we found a collision
                }
            }

            // If the head of the snake is at these coordinates, consider it as under the
            // snake
            if (snakeHead.x == x && snakeHead.y == y) {
                isUnderSnake = true;
            }

        } while (isUnderSnake); // Keep generating new coordinates until it's not under the snake

        // Set the food's position to the new, unoccupied location
        food.x = x;
        food.y = y;
    }
